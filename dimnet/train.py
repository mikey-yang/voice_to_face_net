...
def run_epoch(n_epoch, encoder, classifier, optimizer, epoch):
    ...
    LOGGER.log_epoch(n_epoch, "train",
                     {"EPOCH LOSS": avgloss, "SPEAKER ID LOSS": avgloss_speaker_id, "FACE ID LOSS": avgloss_face})

    wandb.log({"epoch": n_epoch + 1, "loss": avgloss, "original_face": [wandb.Image(dup_face_img)],
               "reconstructed_face": [wandb.Image(dup_gen_img)]})
